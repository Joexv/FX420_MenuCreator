    ...
    public partial class Form1 : Form
    {
                ...
                try
                {
                    ...
                    for (int i = 1; i < c; i++)
                    {
                        for (int a = 1; a <= r; a++)
                        {
                            ...
                            {
                                //Attempt 1 to add prices to the printed menus
                                var costCell = (ws.Cells[a, i + 1] as Range).Value;
                                if (costCell != null)
                                    cellValue = cellValue + "   --$" + costCell;
                                else {
                                    costCell = (ws.Cells[a, i + 2] as Range).Value;
                                    if(costCell != null)
                                        cellValue = cellValue + "   --$" + costCell;
                                }
        ...
        public void PrintFlower()
        {
            ...
            for (int i = 1; i <= Total; i++){
                parts.Add(data["Flower"][i.ToString()] + " $" + data["Cost"][i.ToString()]);}
            parts.Add("");
            parts.Add("---Ounces---");
            var data2 = parser.ReadFile("Ounces.ini");
            Total = Int32.Parse(data2["Settings"]["Total"]);
            for (int i = 1; i <= Total; i++){
                parts.Add(data2["Name"][i.ToString()] + " $" + data2["O_Price"][i.ToString()] + " | $" + data2["H_Price"][i.ToString()]);}
